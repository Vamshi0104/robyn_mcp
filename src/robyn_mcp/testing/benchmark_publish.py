...
def build_benchmark_markdown(robyn_path: str | Path, fastapi_path: str | Path) -> str:
    ...
    rows = []
    metric_names = sorted(
        set((robyn.get("metrics") or {}).keys()) | set((fastapi.get("metrics") or {}).keys())
    )
        ...
        "\n".join(
            [
                ...
                *rows,
                ...
                "Validate methodology before publishing claims.",
            ]
        )
        + "\n"
    )
...
def write_benchmark_markdown(
    robyn_path: str | Path, fastapi_path: str | Path, out_path: str | Path
) -> str:

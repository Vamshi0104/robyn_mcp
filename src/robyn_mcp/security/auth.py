...
def _extract_scopes(
    claims: dict[str, Any], headers: dict[str, str], config: RobynMCPConfig
) -> set[str]:
    scopes: set[str] = set()
    for header_name in config.scopes_header_names:
        ...
        if raw:
            scopes.update(
                {item.strip() for item in str(raw).replace(",", " ").split() if item.strip()}
            )
    claim_value = _first_present(claims, config.scope_claim_keys)
    if isinstance(claim_value, str):
        scopes.update(
            {item.strip() for item in claim_value.replace(",", " ").split() if item.strip()}
        )
...
class HeaderPrincipalResolver(PrincipalResolver):
    def resolve(self, request: Any, headers: dict[str, str], config: RobynMCPConfig) -> AuthContext:
        ...
        if principal_id is None and principal is not None:
            principal_id = (
                getattr(principal, "sub", None) or getattr(principal, "id", None) or str(principal)
            )
